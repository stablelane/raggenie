from collections import OrderedDict
from app.models.request import ConnectionArgument

# Plugin Metadata
__version__ = '1.0.0'
__vectordb_name__ = 'cosmosdb'
__display_name__ = 'Azure Cosmos DB for MongoDB (vCore)'
__description__ = 'CosmosDB for Vector Storage'
__icon__ = '/assets/vectordb/logos/mongodb.svg'
__connection_args__ = [
    {
        "config_type": 1,
        "name": "CosmosDB (vcore) URI",
        "description": "URI to connect to CosmosDB (vcore)",
        "order": 1,
        "required": True,
        "slug": "uri",
        "field": "uri",
        "placeholder": "mongodb+srv://admin:<password>@cluster0.lwqw4z.mongodb.net/",
    }
]





__all__ = [
    __version__, __vectordb_name__, __display_name__ , __description__, __icon__, __connection_args__
]